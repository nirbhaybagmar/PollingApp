...
// QueryQuestionID chains the current query on the "question_id" edge.
func (aq *AnswerQuery) QueryQuestionID() *QuestionQuery {
...
func (aq *AnswerQuery) Clone() *AnswerQuery {
	...
	return &AnswerQuery{
		...
		withQuestionID: aq.withQuestionID.Clone(),
...
// WithQuestionID tells the query-builder to eager-load the nodes that are connected to
// the "question_id" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AnswerQuery) WithQuestionID(opts ...func(*QuestionQuery)) *AnswerQuery {
	query := &QuestionQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withQuestionID = query
	return aq
}
...
func (aq *AnswerQuery) sqlAll(ctx context.Context) ([]*Answer, error) {
	...
	if query := aq.withQuestionID; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Answer, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.QuestionID = []*Question{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Answer)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(answer.QuestionIDPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, aq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "question_id": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "question_id" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.QuestionID = append(nodes[i].Edges.QuestionID, n)
			}
		}
	}
