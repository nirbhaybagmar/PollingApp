...
func (qq *QuestionQuery) QueryUser() *UserQuery {
...
func (qq *QuestionQuery) WithUser(opts ...func(*UserQuery)) *QuestionQuery {
	query := &UserQuery{config: qq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context) ([]*Question, error) {
	...
	if query := qq.withAnswer; query != nil {
		...
		ids := make(map[int]*Question, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Answer = []*Answer{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Question)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(question.AnswerPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, qq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "answer": %w`, err)
		}
		query.Where(answer.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "answer" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Answer = append(nodes[i].Edges.Answer, n)
			}
		}
	}
