...
class NewPoll extends Component {
  ...
  handleSubmit(event) {
    ...
    currDate.setDate(
      currDate.getDate() + parseInt(this.state.pollLength.days, 10)
    );
    currDate.setHours(
      currDate.getHours() + parseInt(this.state.pollLength.hours, 10)
    );

    const pollData = {
      question: {
        body: this.state.question.text,
        user_id: parseInt(localStorage.getItem("user_id"), 10),
        pollExpiry: currDate,
      },
      answers: this.state.choices.map((choice) => {
        return { body: choice.text };
      }),
    };
